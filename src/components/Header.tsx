  ...
  return (
    ...
    >
      ...
      >
        ...
        <nav className="hidden items-center gap-1 xl:flex">
          {NAV.map((n) => (
            <Link
              key={n.href}
              href={n.href}
              className="relative rounded-full px-4 py-2 text-sm font-semibold uppercase tracking-wider text-white/80 transition hover:text-white 2xl:px-5"
            >
        ...
        <button
          type="button"
          className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/20 bg-white/5 text-white xl:hidden"
          onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div className="w-full border-t border-white/10 bg-night-950/95 backdrop-blur-xl xl:hidden">
          <div
